...
private:
	...
	std::vector<VERTEX_F3UI1> m_instances;
...
public:
	...
	SphereRendererDX11(ID3D11Device* pD3DDevice, ID3D11DeviceContext* pD3DDeviceContext, CommonShader* comShader) :
	...
	//Instanceを描画//
	template <class FUNC>
	void DrawFunc(int num_particles, const vec3f* r, const int* color_indexes, const uint32_t* color_table, const VISUAL_SETTING& vis, FUNC func) {
		...
		if (vis.atom) {
			...
			while (i_start < num_particles) {
				...
				for (int i = i_start; i < i_end; ++i) {
					const uint32_t color = color_table[color_indexes[i]];
					const bool is_draw = func(r[i], color);
					if (is_draw){
						if (color & 0xFF000000) {	//完全透過は表示しない//
							m_instances.push_back(VERTEX_F3UI1(r[i], color));
						}
					}
				}
	...
	//Instanceを描画//
	void Draw(int num_particles, const vec3f* r, const int* color_indexes, const uint32_t* color_table, const VISUAL_SETTING& vis) {
		DrawFunc(num_particles, r, color_indexes, color_table, vis, [](vec3f r, uint32_t color) {return true; });
	}
...
private:
	...
	void InitShader() {
		...
		D3D11_INPUT_ELEMENT_DESC layout[] =
		{
			...
			{ "INS_POS", 0, DXGI_FORMAT_R32G32B32_FLOAT, 1, 0, D3D11_INPUT_PER_INSTANCE_DATA, 1 },
			{ "INS_COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 1, 12, D3D11_INPUT_PER_INSTANCE_DATA, 1 },

		};
		UINT numElements = ARRAYSIZE(layout);
		GetAttachedFileName(filepath, _T("vs_inst_color.cso"));
