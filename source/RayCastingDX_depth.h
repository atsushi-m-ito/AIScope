...
protected:
	...
	*/
	...
	ID3D11SamplerState* m_pSampler_depth = nullptr;
	...
	ID3D11Buffer* m_pCB_num_marching;
	ID3D11Buffer* m_pCB_cross_section;


	std::vector<VERTEX_F3F3F3> m_vbo_vertex;

	std::vector<uint32_t> m_vbo_indexes;
	const float* m_current_field = nullptr;
	...
	HRESULT InitShader() {
		...
		GetAttachedFileName(filepath, _T("vs_raycasting_depth.cso"));
		renderer.LoadVertexShader(filepath, "main", "vs_4_1", layout, numElements);


		//Pixel Shader-----------------------------------------

		GetAttachedFileName(filepath, _T("ps_raycasting_depth.cso"));
...

		{
			...
			desc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;// D3D11_FILTER_ANISOTROPIC;// D3D11_FILTER_MIN_MAG_MIP_LINEAR;
			desc.AddressU = D3D11_TEXTURE_ADDRESS_CLAMP;
			desc.AddressV = D3D11_TEXTURE_ADDRESS_CLAMP;//this is needed to create even if 1D.
			desc.AddressW = D3D11_TEXTURE_ADDRESS_WRAP;
			desc.ComparisonFunc = D3D11_COMPARISON_NEVER;
			desc.MinLOD = 0;
			desc.MaxLOD = D3D11_FLOAT32_MAX;
			auto hr = renderer.m_pD3DDevice->CreateSamplerState(&desc, &m_pSampler_depth);
			if (FAILED(hr))
				return hr;
		}
...
#define WITH_MAP
...
#ifdef WITH_MAP
		Texture3DDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE; //for Map
		Texture3DDesc.Usage = D3D11_USAGE_DYNAMIC; //for Map
#else
		Texture3DDesc.CPUAccessFlags = 0;//for UpdateSubresource;
		Texture3DDesc.Usage = D3D11_USAGE_DEFAULT; //for UpdateSubresource
#endif
	...
	bool mUpdateTexture3D(const float* field, int width_x, int width_y, int width_z) {
		if (m_current_field == field) return false;
		...
		renderer.m_pD3DDeviceContext->Map(m_pTexture3D, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
		//GPU固有のピッチのアライメントがあるので、複数回に分けてコピーが必要//
		for (int iz = 0; iz < width_z; ++iz) {
			for (int iy = 0; iy < width_y; ++iy) {
				const int dest_offset = iy * mappedResource.RowPitch + iz * mappedResource.DepthPitch;
				const int src_offset = iy * width_x + iz * width_x * width_y;
				CopyMemory((BYTE*)mappedResource.pData + dest_offset, field + src_offset, 4 * width_x);
			}
		}
		renderer.m_pD3DDeviceContext->Unmap(m_pTexture3D, 0);
#else		

		D3D11_BOX destRegion;
		destRegion.left = 0;
		destRegion.right = width_x* width_y* width_z * 4;
		destRegion.top = 0;
		destRegion.bottom = 1;
		destRegion.front = 0;
		destRegion.back = 1;
		renderer.m_pD3DDeviceContext->UpdateSubresource(m_pTexture3D, 0, nullptr, field, width_x * 4, width_x * width_y * 4);
#endif
...
public:
	...
	RayCastingDX_depth(ID3D11Device* pD3DDevice, ID3D11DeviceContext* pD3DDeviceContext, CommonShader* comShader) :
	...
	void DrawField(const FIELD3D_FRAME* field, ColorRange range, const float* view_matrix_t, const float* projector_matrix_t, int screen_w, int screen_h, ID3D11ShaderResourceView* depthRV) {
		...
		renderer.m_pD3DDeviceContext->PSSetSamplers(2, 1, &m_pSampler_depth);
		...
		renderer.m_pD3DDeviceContext->PSSetShaderResources(2, 1, &depthRV);
		...
		mUpdateTexture3D(field->fieldb, field->grid_x, field->grid_y, field->grid_z);
